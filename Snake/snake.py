...
#For a rectangle u need (top left x,top left y,width,height)
...
while running:
    ...
    #Add head_coord to first index of body_coord list
    #This will essential move all of the snakes body by one position in the list
    #body_coords[x] is the position of the head x frames ago
    #e.g. body_coords[0] is current position of head, body_coords[1] = head position 1 frame ago etc.
    body_coords.insert(0, head_coord)
    body_coords.pop()

    #Update the x,y position of the snakeshead and make a new coordinate
    head_x += snake_dx
    head_y += snake_dy
    head_coord = (head_x, head_y, SNAKE_SIZE, SNAKE_SIZE)
    ...
    if head_rect.left < 0 or head_rect.right > WINDOW_WIDTH or head_rect.top < 0 or head_rect.bottom > WINDOW_HEIGHT or head_coord in body_coords:
        ...
        #Pause the game until the player presses a key, then reset a game
        is_paused = True
        while is_paused:
            for event in pygame.event.get():
